...
describe("Class Calc in 'WAIT_FOR_OPERATOR' state should return", () => {
	let parser, calc;
	...
	test.each([
		['+', 'SUCCESSFULL', new Matrix(matrix), () => token.getPlus()],
		['', 'SUCCESSFULL', new Matrix(), () => token.getEmptyString()],
		[matrixStr, 'ERR_OPERATOR_EXPECTED', new Matrix(matrix), () => token.getArray()],
		['#$@', 'ERR_INVALID_INPUT', new Matrix(matrix), () => token.getError()],
	])(
